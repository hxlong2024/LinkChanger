                        ...
                        if not root_fid: 
                            ...
                            for match in q_matches:
                                ...
                                if new_url:
                                    ...
                                    # 🚀 使用 inject_config 而不是 FIXED_IMAGE_CONFIG
...
def main():
    ...
    with st.sidebar:
        ...
        if inject_config['quark']['enabled']:
            st.success("🖼️ 夸克植入: 开启")
        else:
            st.caption("⚪ 夸克植入: 关闭")
        
        if inject_config['baidu']['enabled']:
    ...
    if not current_job_id:
        ...
        if st.button("🚀 开始转存", type="primary", use_container_width=True):
            ...
            if not cookie_status["quark"] and not cookie_status["baidu"]:
                 st.error("❌ 所有账号 Cookie 均已失效，请联系管理员更新 Secrets 后重试。")
                 return
