...
import threading
import uuid
from urllib.parse import quote # 🆕 用于处理中文通知内容的编码
...
def send_bark_notification(key, title, body):
    if not key: return
    # 对中文进行 URL 编码，防止乱码或发送失败
    encoded_title = quote(title)
    encoded_body = quote(body)
    url = f"https://api.day.app/{key}/{encoded_title}/{encoded_body}?icon=https://cdn-icons-png.flaticon.com/512/2991/2991110.png"
    try:
...
# ==========================================
def worker_thread(job_id, input_text, quark_cookie, baidu_cookie, bark_key): # 🆕 传入 bark_key
    ...
    async def async_worker():
        ...
        try:
            ...
            if bark_key:
                ...
                send_bark_notification(bark_key, title_msg, body_msg)
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        # 🆕 Bark Key 输入框
        bark_key = st.text_input("📢 Bark 推送 Key (可选)", value=get_secret("general", "bark_key"), help="iOS 下载 Bark App，复制链接中的 Key 填入此处。任务完成后手机会收到通知。")
    ...
    if not current_job_id:
        ...
        if st.button("🚀 开始后台转存", type="primary", use_container_width=True):
            ...
            # 🆕 传入 bark_key
            t = threading.Thread(target=worker_thread, args=(new_job_id, input_text, q_c, b_c, bark_key))
            t.start()
